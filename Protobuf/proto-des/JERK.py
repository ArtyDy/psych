...
def min_jerk(pos=None, dur=None, vel=None, acc=None, psg=None):
    ...
    if not np.any(psg):					# passage times unknown, optimize
        if N > 2:
            psg = np.arange(dur/(N-1), dur-dur/(N-1)+1, dur/(N-1)).T
            func1 = lambda psg_: mjCOST(psg_, pos, vel, acc, t0)
            psg = scipy.optimize.fmin(func=func1, x0 = psg)
        else:
            psg = []
...
if __name__ == "__main__":
    ...
    data = pd.read_csv('DataTimeSerie01_extract.txt', delimiter="\t", header=None)
    data.columns = ["ID", "cond1", "cond2", "trial", "nellipse", "x", "y", "v", "vx", "vy", "R", "t"]
    df = data[(data.cond1 == 2) & (data.cond2 == 2) & (data.trial == 1) & (data.nellipse == 1)]
    pos = np.column_stack((df.x.values[::30], df.y.values[::30]))
    np.array(pos.shape[0])
    dur = (df.t.values[-1])
    vel = []
    acc = []
    time = df.t.values[::30]
    psg =[]    #time[1:]
    trj, psgout = min_jerk(pos=pos, dur=dur, vel=vel, acc=acc, psg=psg)
    print(trj)
    #print(J)